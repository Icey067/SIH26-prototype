...
export type BlockStatus =
  ...
  | 'CONTROLLER_APPROVED'
  | 'APPROVED'
  | 'STATION_MASTER_CONCURRED'
  | 'UPCOMING'
  | 'IN_PROGRESS'
  | 'COMPLETED'
  | 'BURSTED'
  | 'SAFETY_TIMEOUT_SUSPENDED'
  | 'MACHINE_STRANDED_OBSTRUCTION'
  | 'EMERGENCY_REVOKED'
  | 'CANCELLED';
...
export interface MaintenanceBlock {
  ...
  controller_private_number?: string;
  station_master_private_number?: string;
  station_master_station?: string;
  controller_approved_at?: string;
  station_master_concurred_at?: string;
  safety_lease_token?: string;
  lease_expires_at?: string;
  traffic_regulation_order?: any;
  is_emergency_regulation?: boolean;
